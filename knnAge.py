...
def ageTest():
	f1 = open('KNNCompareResAgeMuct.txt','w+')		
	

	trainX,trainY = pickle.load(open('MultiPieTrainAge2_XY.p','rb'))
	testX,testY = pickle.load(open('MultiPieValidationAge2_XY.p','rb'))
	
	f1 = open('KnnCompareResAgeMultiPie.txt','w+')		
	...
	joblib.dump(clf,open('age_multipie_knn_calibrated.p','wb'))
	
	
	
	clf = joblib.load(open('age_multipie_knn_calibrated.p', 'rb'))
	...
	print imageNumbers
	with open('knn_age_imageProbabilities.csv', 'wb') as f:
		writer = csv.writer(f)	
	...
	with open('knn_age_thresholds.csv', 'wb') as f:
		writer = csv.writer(f)	
	...
	plt.savefig('rocAgeKNNMultiPie.png')	
	plt.show()
	...
	f1.write('Expression time: %f acc. %f select_1 %f preY:%f testY:%f testX:%f select_2 %f\n'%\
