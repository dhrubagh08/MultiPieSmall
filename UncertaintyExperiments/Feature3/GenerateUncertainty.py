...
def generateUncertaintyValuesInitial():
	...
	for i in range(len(data)):
		...
		for k in range(1,len(data[0])):	
			...
			combinedProbability = float(data[i][k])
...
def generateBins(inputFileName, outputFileName):	
	...
	for i in frange(0.05,1,0.05):
		...
		#print subListDeltaU
		#print sublistClf
		minAvgDelta = 0  # Negative is better. 
		if len(sublistClf)!=0:
			...
			for w in clfList:
				...
				for a,b in zip(subListDeltaU,sublistClf):
					...
					if(b == w):
						sum= sum+ a
						count= count+1
				#print 'sum:%f, count:%d,clf:%s'%(sum,count,w)
				if count !=0:
					avgDelta = float(sum)/count
				else:
					avgDelta =0
				if avgDelta < minAvgDelta:
					#print 'avgDelta:%f, minAvgDelta:%f'%(avgDelta,minAvgDelta)
					maxClf = w
					minAvgDelta = avgDelta
					averageDelta = avgDelta
			'''
			print 'sum:%f, count:%d'%(sum,count)
			averageDelta= float(sum)/count
			'''
			#print 'average delta: %f'%(averageDelta)
			#print maxClf
			finalU.append(i)
