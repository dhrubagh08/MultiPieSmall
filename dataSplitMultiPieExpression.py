...
def getMultiPieValidationDataExpression():
	testX = []
	testY = []
	
	dfp = 'MultiPie_Validation.spkl'
	#rd = pickle.load(open(dfp,'rb'))
	
	ff = open('MultiPie_Expression_Validation_XY.spkl','w')

	count1 = 0 
	with open('smileValidation.csv', 'rb') as f:
		f.next()	# to remove the header of csv file
		reader = csv.reader(f)
		smile_list = [rec for rec in csv.reader(f, delimiter=',')]
		#male_list = list(reader)
	
	print smile_list
	#print not_smile_list
	count =0
	for fu in s_load(open(dfp,'rb')):
		namep = re.split('[_]',fu[0])
		templist = []
		templist.append(namep[0])
		print namep
		if(templist in smile_list):
			testX.append(fu[2])
			testY.append(1)
		else:
			testX.append(fu[2])
	...
	print(testX)
	print(testY)
	#pickle.dump([testX,testY],open('MultiPieValidationGender_XY.p','wb'))
	s_dump([testX,testY],ff)
	
def getMultiPieValidationDataExpression2():	
	testX = []
	testY = []

	
	dfp = 'Multipie_Hog_Small_Train_Memory.pkl'
	rd = pickle.load(open(dfp,'rb'))
	
	count1 = 0 
	with open('smileTraining.csv', 'rb') as f:
		f.next()	# to remove the header of csv file
		reader = csv.reader(f)
		smile_list = [rec for rec in csv.reader(f, delimiter=',')]
		#male_list = list(reader)
	...
	print(testX)
	print(testY)
	pickle.dump([testX,testY],open('MultiPieTrainExpression_XY.p','wb'))
	#s_dump([testX,testY],ff)
...
if __name__ =='__main__':
	getMultiPieValidationDataExpression2()
