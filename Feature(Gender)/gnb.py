...
def glassTest():
	...
	#print trainY
	#print testY
	f1 = open('GnbCompareResGlass.txt','w+')	
	
	
	
	clf_uncalibrated = GaussianNB()
	...
	pickle.dump(clf,open('glass_muct_gnb_calibrated.p','wb'))
	...
	print imageNumbers
	with open('gnb_glass_imageProbabilities.csv', 'wb') as f:
		writer = csv.writer(f)	
...
def genderTest():
	...
	f1 = open('GnbCompareResGenderMultiPie.txt','w+')		
	...
	clf.fit(trainX, trainY)
	#prob_pos_sigmoid = clf_sigmoid.predict_proba(X_test)[:, 1]
	pickle.dump(clf,open('gender_multipie_gnb_calibrated.p','wb'))
	...
	imageNumbers = range(0,len(testY))
	print imageNumbers
	with open('gnb_imageProbabilities.csv', 'wb') as f:
		writer = csv.writer(f)	
		rows = zip(imageNumbers,probX[:,1])
		for row in rows:
			writer.writerow(row)
	...
	with open('gnb_threhsolds.csv', 'wb') as f:
		writer = csv.writer(f)	
